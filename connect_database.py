...
class ConnectDatabase:
    ...
    #try:
        #    with self.conn.cursor() as cursor:
        #        if self.delete_exists(del_productcode,del_seasons,del_brand_comboBox,del_color):
        #            
        #            # Execute the SQL query with the provided data
        #            cursor.execute(update_sql, (del_quantity, del_sale_price, del_productcode, del_seasons, del_brand_comboBox, del_color,del_quantity))
        #            self.conn.commit()
        #            QMessageBox.information(None, "Success", "Data deleted successfully.", QMessageBox.StandardButton.Ok)
